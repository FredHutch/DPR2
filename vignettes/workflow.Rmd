...
```{r yaml}
cat(readLines(paste0('datapackager.yml')), sep = '\n')
```
...
dd_file <- list.files(paste0('inst', '\\','data_digest'),full.names = TRUE)
dd_file
```

### Examine the data digest

```{r examine-data-digest, eval=FALSE}
...
```
### Next Steps
- **Version controlling data packages**
    ...
    3. Push your local package repository to `github`. [see this step](https://help.github.com/articles/adding-an-existing-project-to-github-using-the-command-line/)
