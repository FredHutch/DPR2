...
The DPR2 package `datapackager.yml` and `DESCRIPTION` files can be initialized with specific values using the `yaml` and `desc` arguments. The arguments are passed lists that are generated using `dpr_yaml_init()` and `dpr_description_init()` function calls. The arguments to these calls are simply key value pairs that are added to the appropriate configuration file, whereas these functions also add the necessary key value pairs that are required but not enumerated. For a list of default values for each of these, see `?dpr_yaml_defaults` and `?dpr_description_defaults`.

By default, DPR2 is initialized using renv for dependency version control. DPR2 can help tackle problems of unstable and non-reproducible environments by using renv after initializing a DPR2 package, using `renv::init()`. Once the data package is initialized with renv, renv will make sure when a dependency is installed by a user, it will use the same version of that dependency when building the data package in the future, and will inform users whether they need to install a particular version of a package dependency. For more details on `renv`, refer to the [renv documentation](https://docs.posit.co/ide/user/ide/guide/environments/r/renv.html).
