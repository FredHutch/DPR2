...
#' * `write_data_docs`       : `TRUE`, if data documentation should be generated when whendering
#' * `to_build_directory`    : `inst/to_build`, the location of the tracking directory for what script are built and objects are saved.
...
#' * `build_output`          : `../`, where the package should be save if the tarball is built relative to the package root
#'
...
#' * `objects`                : which objecsts to save from the processing scripts environmetnts. These values overwrite what objects have been assigned in `inst/to_build/objects` with `dpr_add_objects`.
